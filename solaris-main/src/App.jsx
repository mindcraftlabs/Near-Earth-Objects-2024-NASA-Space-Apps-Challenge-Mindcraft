...
const SolarSystem = () => {
  ...
  useEffect(() => {
    ...
    planets.forEach(planet => {
      ...
      planetsRef.current[planet].add(sound);
    });
  }, []);
  ...
  useEffect(() => {
    if (experienceStarted) {
      // Start the audio
      planets.forEach(planet => {
        const sound = planet !== "sat" ? planetsRef.current[planet].children[0] : planetsRef.current[planet].children[1];
        if (sound?.play) {
          sound.play();
        }
      });
